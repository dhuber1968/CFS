function FitExp2Data
    ...
    RTErrors=[149.53232 98.29219 128.4517]; % witin subject standard error (Cousinea Morey method)
    ...
    params=ones(1,7);
    params=[-0.7118   -2.3812   -1.7475   -2.0357   -3.9365   -1.2395]; % initial parameter values
   ...
   [(1 - normcdf(0, params(1), 1)) exp(params(2:6)) ]   % convert best-fitting parameters from gammas to probabilities
    ...
    function err=CalcError(params)
        ...
        Gam_IC  =  params(5);       % both ends of illusory rectangle (or collinearity)
        ...
        for cond=1:4
          ...
          %      ILC = exp(Gam_IC); % include for collinearity model
          %      IRC = exp(Gam_IC); % include for collinearity model

                ALL= exp(Gam_X);   % include for shapes model

        % include the following for salience 
            ...
            elseif cond==3 % left talk condition
                ILC = exp(Gam_IC);      % include for collinearity or shapes
        ...
        % include the following for salience 
            ...
            elseif cond==4 % right talk condition
                IRC = exp(Gam_IC);       % include for collinearity or shapes
        ...
        % include the following for salience 
          ...
          if Plot==1
            ...
            for c=1:4
                ...
                if cond==1
                    Hdata=plot(FreqData(RespOrder,cond)./N(cond),'ob','LineWidth', 2,'MarkerSize',4);
                    hold on
                    errorbar([1:15],FreqData(RespOrder,cond)./N(cond),Errors(:,cond),Errors(:,cond),'LineStyle','none','LineWidth', 2,'Color','b');
                else
                    Hdata=plot(FreqData(RespOrder,cond)./N(cond)-FreqData(RespOrder,1)./N(1),'ob','LineWidth', 2,'MarkerSize',4);
                    hold on
                ...
                if cond==1
                    Hmodel=plot(FreqPred(RespOrder,cond)./N(cond),'xr','LineWidth',2,'MarkerSize',8);
                else
                    Hmodel=plot(FreqPred(RespOrder,cond)./N(cond)-FreqPred(RespOrder,1)./N(1),'xr','LineWidth',2,'MarkerSize',8);
                end

                set(gca,'FontName','Arial','FontSize',8,'FontWeight','Bold',  'LineWidth', 2);
                if cond==1
                    axis([.5 15.5 0 .4]);
                    line([4.5 4.5],[0 .4]);
                    line([8.5 8.5],[0 .4]);
                ...
                LegInclude=[Hdata Hmodel];
                legend(LegInclude,'Data','Model','Location','NorthWest');
                if cond==1
                    title('Both Hate');
                elseif cond==2
                    title('Both Talk');
                ...
                for i=1:15
                    text(i, ax.YLim(1), sprintf('%s\n%s\n%s', myLabels{:,i}), ...
                            'horizontalalignment', 'center', 'verticalalignment', 'top','FontName','Arial','FontSize',8,'FontWeight','Bold');  
                end

                if cond==1
                    ylabel('Report Probability');
                else
                    ylabel('Change from Both Hate');
                end
                
                filename=sprintf('Cond%d.bmp',cond);
                axes('pos',[.4 (.86-(c-1)*.22) .08 .08])
                imshow(filename);
            ...
            Hdata=plot(RTs([1 3 2]),'ob','LineWidth', 2,'MarkerSize',8);
            hold on
            errorbar([1:3],RTs([1 3 2]),RTErrors([1 3 2]),RTErrors([1 3 2]),'LineStyle','none','LineWidth', 2,'Color','b');

            Hmodel=plot(RTPred([1 3 2]),'xr','LineWidth',2,'MarkerSize',16);

            set(gca,'FontName','Arial','FontSize',12,'FontWeight','Bold',  'LineWidth', 2);
            axis([.5 3.5 2100 2900]);
            LegInclude=[Hdata Hmodel];
            legend(LegInclude,'Data','Model','Location','SouthWest');
             ax = gca;
             ax.XTick = [1:3];
             ax.XTickLabel = ({'Both Hate' 'Mixed' 'Both Talk'});
             ax.YTick = [2200:200:3000];
             ylabel('Median Breakthrough Time (ms)');
             xlabel('Condition');
            ...
            G2Report=-2*sum(log(mnpdf(FreqData',ProbPred')./mnpdf(FreqData',ProbData')))
            G2RT=-2*sum(log(normpdf(RTPred(:),RTs(:),RTErrors(:))./normpdf(RTs(:),RTs(:),RTErrors(:))))
    ...
    %    err=err-2*(28*70)*sum(log(normpdf(RTPred(:),RTs(:),RTSD(:))./normpdf(RTs(:),RTs(:),RTSD(:))));
        err=err-2*sum(log(normpdf(RTPred(:),RTs(:),RTErrors(:))./normpdf(RTs(:),RTs(:),RTErrors(:))));
